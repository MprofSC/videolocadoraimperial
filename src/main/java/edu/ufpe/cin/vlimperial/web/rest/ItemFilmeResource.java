...
public class ItemFilmeResource {
    ...
    /**
     * SEARCH  /_search/item-filmes?query=:query : search for the itemFilme corresponding
     * to the query.
     *
     * @param query the query of the itemFilme search
     * @return the result of the search
     */
    @GetMapping("/_search/item-filmes")
    @Timed
    public List<ItemFilme> searchItemFilmes(@RequestParam String query) {
        log.debug("REST request to search ItemFilmes for query {}", query);
        return itemFilmeService.search(query);
    }
