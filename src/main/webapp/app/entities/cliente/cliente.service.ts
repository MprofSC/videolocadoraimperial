...
export class ClienteService {
    ...
    protected convertDateFromClient(cliente: ICliente): ICliente {
        const copy: ICliente = Object.assign({}, cliente, {
            dataNascimento: cliente.dataNascimento != null && cliente.dataNascimento.isValid() ? cliente.dataNascimento.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((cliente: ICliente) => {
                cliente.dataNascimento = cliente.dataNascimento != null ? moment(cliente.dataNascimento) : null;
            });
        }
        return res;
    }
