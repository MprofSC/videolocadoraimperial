...
public class FilmeResourceIntTest {
     ...
     *
     * @see edu.ufpe.cin.vlimperial.repository.search.FilmeSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createFilme() throws Exception {
        ...
        // Validate the Filme in Elasticsearch
        verify(mockFilmeSearchRepository, times(1)).save(testFilme);
    }
    ...
    public void createFilmeWithExistingId() throws Exception {
        ...
        // Validate the Filme in Elasticsearch
        verify(mockFilmeSearchRepository, times(0)).save(filme);
    }
    ...
    public void updateFilme() throws Exception {
        ...
        reset(mockFilmeSearchRepository);
    ...
    public void deleteFilme() throws Exception {
        ...
        // Validate the Filme in Elasticsearch
        verify(mockFilmeSearchRepository, times(1)).deleteById(filme.getId());
    }

    @Test
    @Transactional
    public void searchFilme() throws Exception {
        // Initialize the database
        filmeService.save(filme);
        when(mockFilmeSearchRepository.search(queryStringQuery("id:" + filme.getId())))
            .thenReturn(Collections.singletonList(filme));
        // Search the filme
        restFilmeMockMvc.perform(get("/api/_search/filmes?query=id:" + filme.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(filme.getId().intValue())))
            .andExpect(jsonPath("$.[*].tituloOriginal").value(hasItem(DEFAULT_TITULO_ORIGINAL)))
            .andExpect(jsonPath("$.[*].tituloPortugues").value(hasItem(DEFAULT_TITULO_PORTUGUES)))
            .andExpect(jsonPath("$.[*].paises").value(hasItem(DEFAULT_PAISES)))
            .andExpect(jsonPath("$.[*].ano").value(hasItem(DEFAULT_ANO)))
            .andExpect(jsonPath("$.[*].direcao").value(hasItem(DEFAULT_DIRECAO)))
            .andExpect(jsonPath("$.[*].elenco").value(hasItem(DEFAULT_ELENCO)))
